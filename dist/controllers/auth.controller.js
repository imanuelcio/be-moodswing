...
import { formatError, UnauthorizedError, ValidationError, } from "../core/errors.js";
import StoreCookieInResponse from "../utils/jwt.js";
import { setCookie } from "hono/cookie";
import { createApiKeySchema, nonceSchema, verifySchema, } from "../schemas/auth.schema.js";
function getHostFromRequest(c) {
...
export class AuthController {
    ...
    async generateNonce(c) {
        try {
            ...
            const { nonce, message, expiresInSec } = await this.authService.generateNonceForWallet(parsed.address, parsed.chainKind, domain);
            if (!nonce || !message) {
            ...
            if (error instanceof z.ZodError) {
                return c.json(formatError(new ValidationError("Invalid input", error)), 400);
            }
    ...
    safeLogger(c) {
        ...
        return {
            info: (obj, msg) => l?.info ? l.info(obj, msg) : console.info(msg ?? "", obj),
            error: (obj, msg) => l?.error ? l.error(obj, msg) : console.error(msg ?? "", obj),
        };
    }
    async verifySignature(c) {
        try {
            ...
            if (!domain)
                throw new ValidationError("Domain is required");
            const { token, user, wallet } = await this.authService.verifyWalletSignature({
            ...
            logger.info({ userId: user.id, address: wallet.address, chain: wallet.chainKind }, "User authenticated");
            ...
            const status = err instanceof z.ZodError
                ...
                : err instanceof ValidationError
                    ...
                    : err instanceof UnauthorizedError
                        ? 401
                        : 500;
            return c.json(formatError(err), status);
        }
    }
    async createApiKey(c) {
        try {
