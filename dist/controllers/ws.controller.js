...
export class WSController {
    async upgradeToWebSocket(c) {
        try {
            const token = c.req.query("token") ||
                c.req.header("Authorization")?.replace("Bearer ", "");
            if (!token) {
                return c.json(formatError(new Error("Authentication token required")), 401);
            }
            ...
            return c.json({
                ...
                instructions: "Connect to /realtime with token parameter or Authorization header",
            });
        }
        catch (error) {
