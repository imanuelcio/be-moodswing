...
import { awardPointsSchema, bulkAwardSchema, transferPointsSchema, } from "../schemas/point.schema.js";
export class PointsController {
    pointsService;
    constructor(pointsService = new PointsService()) {
        this.pointsService = pointsService;
    }
    async getUserBalance(c) {
        try {
    ...
    async transferPoints(c) {
        try {
            ...
            const logger = c.get("logger");
            logger.info({ fromUserId: userId, toUserId, amount }, "Points transferred");
    ...
    async awardPoints(c) {
        try {
            const body = await c.req.json();
            const { userId, amount, reason, metadata } = awardPointsSchema.parse(body);
