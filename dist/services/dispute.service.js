import { DisputeRepository, } from "../repo/dispute.repo.js";
...
export class DisputeService {
    ...
    marketRepo;
    pointsRepo;
    constructor(disputeRepo = new DisputeRepository(), disputeVoteRepo = new DisputeVoteRepository(), resolutionRepo = new ResolutionRepository(), marketRepo = new MarketRepository(), pointsRepo = new PointsRepository()) {
        this.disputeRepo = disputeRepo;
        this.disputeVoteRepo = disputeVoteRepo;
        this.resolutionRepo = resolutionRepo;
        this.marketRepo = marketRepo;
        this.pointsRepo = pointsRepo;
    }
    async createDispute(marketId, userId, request) {
        ...
        if (userBalance < minimumPointsRequired) {
            throw new ValidationError(`Minimum ${minimumPointsRequired} points required to open dispute. Current balance: ${userBalance}`);
        }
        ...
        const hasActiveDispute = activeDisputes.some((d) => ["open", "voting"].includes(d.status));
        if (hasActiveDispute) {
        ...
        if (hoursDiff > disputeWindowHours) {
            throw new ValidationError(`Dispute window has expired. Must dispute within ${disputeWindowHours} hours of resolution.`);
        }
        try {
    ...
    async voteOnDispute(disputeId, userId, request) {
        const { vote } = request;
        ...
        const existingVote = await this.disputeVoteRepo.findByDisputeAndUser(disputeId, userId);
        if (existingVote) {
        ...
        if (userBalance < minimumPointsToVote) {
            throw new ValidationError(`Minimum ${minimumPointsToVote} points required to vote. Current balance: ${userBalance}`);
        }
        ...
        try {
            ...
            logger.error({ error, disputeId, userId, vote }, "Failed to vote on dispute");
            throw error;
        }
    }
    async checkDisputeAutoResolution(disputeId) {
        ...
        if (voteSummary.totalVotes < minimumVotesRequired ||
    ...
    async resolveDispute(disputeId, outcome, source = "admin", resolvedBy) {
        const dispute = await this.disputeRepo.findWithDetails(disputeId);
        if (!dispute) {
            throw new NotFoundError("Dispute", disputeId);
        }
    ...
    async processDisputeRewards(disputeId, outcome) {
        ...
        // Award rewards
        if (rewards.length > 0) {
            await this.pointsRepo.bulkAddPoints(rewards.map((reward) => ({
    ...
    async getActiveDisputes() {
        ...
        // Add vote summaries
        const disputesWithVotes = await Promise.all(disputes.map(async (dispute) => {
            const voteSummary = await this.disputeVoteRepo.getVoteSummary(dispute.id);
