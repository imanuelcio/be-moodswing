...
export class PositionService {
    ...
    outcomeRepo;
    pointsRepo;
    constructor(positionRepo = new PositionRepository(), marketRepo = new MarketRepository(), outcomeRepo = new OutcomeRepository(), pointsRepo = new PointsRepository()) {
    ...
    async getUserPositions(userId, params = {}) {
        ...
        const positionsWithPnL = await Promise.all(positions.map(async (position) => {
    ...
    async closePosition(positionId, userId, params = {}) {
        const position = await this.positionRepo.findById(positionId);
        if (!position) {
            throw new NotFoundError("Position", positionId);
        }
        ...
        const closePrice = price || (await this.getCurrentPrice(position.outcome_id));
        // Calculate P&L
        ...
        await writeToOutbox({
            ...
            payload: {
                ...
                message: `Position closed with ${realizedPnL > 0 ? "profit" : "loss"}: ${realizedPnL.toFixed(2)}`,
                position_id: positionId,
            },
        });
        return { position: updatedPosition, pointsEntry };
    }
    async getCurrentPrice(outcomeId) {
    ...
    async liquidatePositions(marketId, outcomeId) {
        ...
        for (const position of positions) {
            if ((position.qty_points || 0) > 0 ||
                (position.qty_token_amount || 0) > 0) {
                await this.positionRepo.liquidatePosition(position.user_id, marketId, outcomeId);
                liquidatedCount++;
            }
        }
