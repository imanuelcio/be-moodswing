...
import { ValidationError, ConflictError, NotFoundError, ForbiddenError, } from "../core/errors.js";
import { eventTypes, topics } from "../core/topics.js";
import { writeToOutbox } from "../core/outbox.js";
export class MarketService {
    marketRepo;
    outcomeRepo;
    constructor(marketRepo = new MarketRepository(), outcomeRepo = new OutcomeRepository()) {
        this.marketRepo = marketRepo;
        this.outcomeRepo = outcomeRepo;
    }
    async createMarket(creatorUserId, request) {
        ...
        // Validate dates
        this.validateMarketDates(marketData.open_at, marketData.close_at, marketData.resolve_by);
        // Create market
        const createData = {
            ...marketData,
            slug,
        ...
        // Create outcomes
        const outcomeCreateData = outcomes.map((outcome) => ({
            market_id: market.id,
            key: outcome.key,
            name: outcome.name,
            initial_price: outcome.initial_price || 0.5,
        }));
        const createdOutcomes = await this.outcomeRepo.createBatch(outcomeCreateData);
    ...
    async updateMarket(marketId, userId, request) {
        ...
        if (request.open_at || request.close_at || request.resolve_by) {
            this.validateMarketDates(request.open_at || market.open_at, request.close_at || market.close_at, request.resolve_by || market.resolve_by);
        }
    ...
    async getMarketBySlug(slug) {
        ...
        const marketWithOutcomes = await this.marketRepo.findWithOutcomes(market.id);
    ...
    async getMarketWithStats(marketId) {
        const market = await this.getMarket(marketId);
        const stats = await this.marketRepo.getMarketStats(marketId);
        const outcomesWithStats = await this.outcomeRepo.getOutcomesWithStats(marketId);
        return {
            ...market,
    ...
    async deleteMarket(marketId, userId) {
        const market = await this.marketRepo.findById(marketId);
        if (!market) {
            throw new NotFoundError("Market", marketId);
        }
    ...
    async openMarket(marketId, userId) {
        const market = await this.marketRepo.findById(marketId);
        if (!market) {
            throw new NotFoundError("Market", marketId);
        }
        ...
        if (openAt > now) {
            throw new ValidationError("Cannot open market before scheduled open time");
        }
        return this.updateMarket(marketId, userId, { status: "open" });
    }
    async closeMarket(marketId, userId) {
        const market = await this.marketRepo.findById(marketId);
        if (!market) {
            throw new NotFoundError("Market", marketId);
        }
    ...
    validateMarketDates(openAt, closeAt, resolveBy) {
        const now = new Date();
        if (openAt) {
