...
function safePublish(publish, payload) {
    try {
        publish(payload);
    }
    catch (e) {
...
export async function ensureHermesPriceStream(priceId, symbol, publish) {
    ...
    const start = async () => {
        ...
        const url = `https://hermes.pyth.network/v2/updates/price/stream` +
...
// hermes-latest.ts
export async function fetchHermesLatest(priceId) {
...
function scaleDecimal(intStr, expo) {
...
export function extractFromParsed(msg) {
    const node = (Array.isArray(msg?.parsed) && msg.parsed[0]) ??
    ...
    if (!pObj || pObj.price == null || pObj.expo == null)
        return null;
    ...
    const tsSec = Number(pObj.publish_time ?? msg?.publishTime ?? Date.now() / 1000);
    return {
        id,
