...
import { formatError, UnauthorizedError, ForbiddenError, } from "../core/errors.js";
...
export function createJwtMiddleware() {
    return async (c, next) => {
        try {
            ...
            if (!tokenFromCookies) {
                return c.json(formatError(new UnauthorizedError("Missing JWT token")), 401);
            }
            ...
            return c.json(formatError(err instanceof Error ? err : new Error(String(err))), 401);
        }
    };
}
...
export function createApiKeyMiddleware() {
    return async (c, next) => {
        try {
            ...
            if (!apiKey) {
                return c.json(formatError(new UnauthorizedError("Missing API key")), 401);
            }
...
export function createAdminMiddleware() {
    return async (c, next) => {
        try {
            ...
            if (!token) {
                return c.json(formatError(new UnauthorizedError("Missing or invalid authorization")), 401);
            }
            const payload = verifyJwtToken(token);
            // Set user context
            c.set("userId", payload.userId);
