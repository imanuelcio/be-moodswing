...
import {
  ...
  findMetadataPda,
  fetchMetadata,
  type Metadata, // <- type-only import OK
...
import {
  ...
  listTokensWithJoins,
  upsertToken,
  getMintedCountByCollectionId,
  listCollections,
} from "../repo/nft.repo.js";
...
export class NftService {
  static async verifyAndRecordMint(
    params: MintRecordInput & { userId: number }
  ) {
    ...
    if (!top?.address) throw new Error("No token accounts for this mint");
    ...
    const parsed = parsedAcc.value?.data as any;
    const holder = parsed?.parsed?.info?.owner as string | undefined;
    ...
    let onChainCollectionMint: string | null = null;

    const coll: any = (metadata as any).collection;
    if (coll) {
      // UMI Option style
    ...
    let offchainMeta: any = null;
    try {
      const rawUri: string =
        (metadata as any).uri ?? (metadata as any).data?.uri ?? "";
    ...
    const onchainName: string = (
      (metadata as any).name ??
      (metadata as any).data?.name ??
      ""
    )
      .replace(/\0/g, "")
      .trim();
    const onchainSymbol: string = (
      (metadata as any).symbol ??
      (metadata as any).data?.symbol ??
      ""
    )
      .replace(/\0/g, "")
      .trim();
    const onchainUri: string = (
      (metadata as any).uri ??
      (metadata as any).data?.uri ??
      ""
    )
      .replace(/\0/g, "")
      .trim();
  ...
  static async publicGetCollectionDetail(collectionId: number) {
  ...
  static async ownedListTokens(
    userId: number,
    query: Omit<ListTokensQuery, "collectionId"> & { collectionId?: number }
  ) {
  ...
  static async publicListTokens(query: ListTokensQuery) {
    return await listTokensWithJoins(query);
  }
  static async listTokens(query: ListTokensQuery) {
