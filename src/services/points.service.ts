import {
  PointsRepository,
  type CreatePointsEntryData,
} from "../repo/points.repo.js";
import { ValidationError, NotFoundError } from "../core/errors.js";
import { eventTypes, topics } from "../core/topics.js";
import { writeToOutbox } from "../core/outbox.js";

export class PointsService {
  constructor(private pointsRepo = new PointsRepository()) {}
  ...
  async deductPoints(
    ...
    if (currentBalance < amount) {
      throw new ValidationError(
        `Insufficient points. Balance: ${currentBalance}, Required: ${amount}`
      );
    }
  ...
  async transferPoints(
    ...
    if (fromBalance < amount) {
      throw new ValidationError(
        `Insufficient points. Balance: ${fromBalance}, Required: ${amount}`
      );
    }
  ...
  async awardDailyBonus(userId: string): Promise<void> {
  ...
  async bulkAwardPoints(
    ...
    const entries: CreatePointsEntryData[] = awards.map((award) => ({
