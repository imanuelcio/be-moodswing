...
function resolveChainKey(chainKind: "ethereum" | "solana"): string {
...
// ===============================
async function upsertUserAndWallet(
  address: string,
  ...
  if (existingWallet?.user_id) {
    ...
    if (!user?.id) throw new Error("Failed to create/find user");
    userId = user.id;
  ...
  if (!finalWallet) throw new Error("Wallet upsert failed");

  return {
    userId,
    walletId: finalWallet.id as number,
    handle: undefined as string | undefined, // kalau butuh handle, fetch user lagi di sini
  };
}
...
export class AuthService {
  constructor(
    ...
    private chainRepo = new ChainRepository(),
    private walletRepo = new WalletRepository()
  ) {}

  async generateNonceForWallet(
    address: string,
    chainKind: "ethereum" | "solana",
    domain: string
  ) {
  ...
  async verifyWalletSignature(params: {
    ...
    if (!raw) throw new UnauthorizedError("Nonce not found or expired");
  ...
  async revokeApiKey(keyId: string, ownerUserId: string): Promise<void> {
  ...
  async listApiKeys(ownerUserId: string): Promise<
    Array<{
      id: string;
      plan: string;
      rateLimitPerHour: number;
