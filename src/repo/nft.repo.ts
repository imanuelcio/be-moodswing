...
export type TokenListFilters = {
  collectionId?: number;
  ownerUserId?: number;
  order?: "minted_at_desc" | "minted_at_asc";
  page?: number; // 1-based
...
export async function listTokensWithJoins(filters: TokenListFilters) {
...
export async function getMintedCountByCollectionId(collectionId: number) {
...
export async function getCollectionByContractAddress(addr: string) {
  const { data, error } = await supabase
    .from("nft_collections")
    .select(
      "id, chain_id, contract_address, symbol, name, royalties_bps, revenue_share_pct, created_at"
...
export async function upsertToken(params: {
  ...
  token_id: string; // mintAddress (Solana)
  owner_user_id: number | null;
  minted_at: string | null; // ISO
  metadata?: any;
}) {
  const { data, error } = await supabase
    .from("nft_tokens")
