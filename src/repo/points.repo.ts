...
export interface CreatePointsEntryData {
  user_id: string;
  reason: string;
  delta: number;
  ref_type?: string;
...
export class PointsRepository {
  ...
  async addPoints(
    entryData: CreatePointsEntryData
    ...
    return executeQuery<PointsLedgerEntry>(
      supabase
        .from("points_ledger")
        .insert({
          ...entryData,
  ...
  async getLeaderboard(
    ...
    if (error) {
      ...
      if (fallbackError) {
        throw new Error(
          `Failed to get points leaderboard: ${fallbackError.message}`
        );
      }
