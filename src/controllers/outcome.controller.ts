...
import {
  createOutcomeSchema,
  updateOutcomeSchema,
} from "../schemas/outcome.schema.js";
...
export class OutcomeController {
  ...
  async createOutcome(c: Context) {
    try {
      ...
      if (market.creator_user_id !== userId) {
        return c.json(
          formatError(
            new ForbiddenError("Only market creator can add outcomes")
          ),
          403
        );
      }
      ...
      if (market.status !== "draft") {
        return c.json(
          formatError(
            new ValidationError("Can only add outcomes to draft markets")
          ),
          400
        );
      }
      ...
      const existingOutcome = await this.outcomeRepo.findByMarketAndKey(
        marketId,
        key
      );
      if (existingOutcome) {
        return c.json(
          formatError(
            new ValidationError(`Outcome with key '${key}' already exists`)
          ),
          409
        );
      }
      ...
      logger.info(
        { userId, marketId, outcomeId: outcome.id },
        "Outcome created"
      );
  ...
  async updateOutcome(c: Context) {
    try {
      ...
      if (market.creator_user_id !== userId) {
        return c.json(
          formatError(
            new ForbiddenError("Only market creator can update outcomes")
          ),
          403
        );
      }
      ...
      if (market.status !== "draft") {
        return c.json(
          formatError(
            new ValidationError("Can only update outcomes in draft markets")
          ),
          400
        );
      }
      ...
      const body = await c.req.json();
      const updateData = updateOutcomeSchema.parse(body) as UpdateOutcomeData;

      const updatedOutcome = await this.outcomeRepo.update(
        outcomeId,
        updateData
      );
  ...
  async deleteOutcome(c: Context) {
    try {
      ...
      if (market.creator_user_id !== userId) {
        return c.json(
          formatError(
            new ForbiddenError("Only market creator can delete outcomes")
          ),
          403
        );
      }
      ...
      if (market.status !== "draft") {
        return c.json(
          formatError(
            new ValidationError("Can only delete outcomes from draft markets")
          ),
          400
        );
      }
      ...
      if (allOutcomes.length <= 2) {
        return c.json(
          formatError(
            new ValidationError("Market must have at least 2 outcomes")
          ),
          400
        );
      }
  ...
  async getOutcome(c: Context) {
    try {
      ...
      if (!outcome) {
        return c.json(
          formatError(new NotFoundError("Outcome", outcomeId)),
          404
        );
      }
