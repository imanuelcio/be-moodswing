...
export class WalletController {
  ...
  async getPrimaryWallet(c: Context) {
    try {
      const userId = c.get("userId");
      if (!userId) {
        return c.json(
  ...
  async addWallet(c: Context) {
    try {
      ...
      const existingWallet = await this.walletRepo.findByAddress(
        address,
        chain_id
      );
      if (existingWallet && existingWallet.user_id === userId) {
        return c.json(
          formatError(
            new ValidationError("Wallet already exists for this user")
          ),
          409
        );
      }
  ...
  async updateWallet(c: Context) {
    try {
      ...
      const body = await c.req.json();
      const walletData = updateWalletSchema.parse(body) as UpdateWalletData;
  ...
  async getWalletStats(c: Context) {
    try {
      const userId = c.get("userId");
