...
export class DisputeController {
  constructor(private disputeService = new DisputeService()) {}
  ...
  async createDispute(c: Context) {
    try {
      ...
      const request = createDisputeSchema.parse(body) as CreateDisputeRequest;
  ...
  async voteOnDispute(c: Context) {
    try {
      ...
      const body = await c.req.json();
      const request = voteOnDisputeSchema.parse(body) as VoteOnDisputeRequest;

      const vote = await this.disputeService.voteOnDispute(
        disputeId,
        userId,
        request
      );
  ...
  async listDisputes(c: Context) {
    try {
      ...
      const { status, marketId, openedBy, page, limit, order_by, order_dir } =
        params;
  ...
  async getActiveDisputes(c: Context) {
    try {
  ...
  async getDisputeVotes(c: Context) {
    try {
      const disputeId = c.req.param("id");
  ...
  async updateVote(c: Context) {
    try {
      ...
      const dispute = await this.disputeService.getDispute(disputeId);
      const userVote = dispute.dispute_votes?.find(
        (v: any) => v.id === voteId && v.user_id === userId
      );

      if (!userVote) {
        return c.json(
          formatError(
            new ValidationError("Vote not found or not owned by user")
          ),
          404
        );
      }
      ...
      return c.json(
        formatError(
          new ValidationError("Vote updates not allowed after submission")
        ),
        400
      );
    } catch (error) {
