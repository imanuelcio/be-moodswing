...
} from "../schemas/resolution.schema.js";

export class ResolutionController {
  constructor(private resolutionService = new ResolutionService()) {}

  async resolveMarket(c: Context) {
    try {
      const marketId = c.req.param("marketId");
      const userId = c.get("userId"); // From admin middleware

      const body = await c.req.json();
      const request = resolveMarketSchema.parse(body) as ResolveMarketRequest;
  ...
  async getMarketResolution(c: Context) {
    try {
      const marketId = c.req.param("marketId");

      const resolution = await this.resolutionService.getMarketResolution(
        marketId
      );
  ...
  async listResolutions(c: Context) {
    try {
  ...
  async updateResolution(c: Context) {
    try {
      const resolutionId = c.req.param("id");
      ...
      const resolution = await this.resolutionService.updateResolution(
        resolutionId,
        updateData,
        userId
      );
  ...
  async validateResolution(c: Context) {
    try {
      const marketId = c.req.param("marketId");

      const validation =
        await this.resolutionService.validateResolutionEligibility(marketId);
  ...
  async getResolutionStats(c: Context) {
    try {
      const query = c.req.query();
